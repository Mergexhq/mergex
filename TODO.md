...
| 4.3 | Replace fake `send()` function in `ChatCard` with real API call | high | complete | `POST /api/chat` route created at `src/app/api/chat/route.ts`. Calls `runEngine()` via `createProviderRegistry()`. `ChatCard.send()` in `InteractiveExperiences.tsx` rewritten with `fetch`, try/catch/finally, and real `response.content` display. All hardcoded replies removed |
...
| 4.6 | Add conversation context (multi-turn) | medium | complete | API route sends `history: []` for now (engine already supports `history: ChatMessage[]`). Full multi-turn history pass-through is the natural next step - requires UI serialisation of `Message[]` into `ChatMessage[]` |
